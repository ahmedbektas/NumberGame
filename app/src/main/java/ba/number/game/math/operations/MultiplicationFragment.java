...
public class MultiplicationFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mActivity.getSupportActionBar().setTitle("Answer question " + MultiplicationActivity.questionCounter);

        int oldScore = Prefs.getInstance(getActivity()).getScore(Prefs.MULTIPLICATION_QUESTION_SCORE);

        Log.i("QuestionFragment", "oldScore: " + oldScore);

        mActivity.getSupportActionBar().setSubtitle("Score: " + (((MultiplicationActivity) mActivity).getTrueCounter() * scoreMultiplier) + "     Total score: " + (oldScore + (((MultiplicationActivity) mActivity).getTrueCounter() * scoreMultiplier)));
        ...
        nextBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (MultiplicationActivity.questionCounter<10) {
                    ((MultiplicationActivity) mActivity).ubaciFragment();
                    ...
                    scoreDialog.setTitle("Score: " + ((MultiplicationActivity) mActivity).getTrueCounter()*scoreMultiplier);
                    scoreDialog.setMessage("True answers: " + ((MultiplicationActivity) mActivity).getTrueCounter() + "\nFalse answers: " + (10-((MultiplicationActivity) mActivity).getTrueCounter()));
                    scoreDialog.setNegativeButton("OK", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            int oldScore = Prefs.getInstance(getActivity()).getScore(Prefs.MULTIPLICATION_QUESTION_SCORE);
                            int totalScore = oldScore + (((MultiplicationActivity) mActivity).getTrueCounter()*scoreMultiplier);

                            Prefs.getInstance(getActivity()).insertScore(Prefs.MULTIPLICATION_QUESTION_SCORE, totalScore);

                            getActivity().setResult(getActivity().RESULT_OK, new Intent().putExtra("score", totalScore));

                            ((MultiplicationActivity) mActivity).setTrueCounter(0);
